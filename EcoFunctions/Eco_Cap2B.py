...
Updated: 2025-03-05 - ADDED: Finite water depth support via sea_bottom parameter
         Default sea_bottom=-2.4 m (tank depth). Use sea_bottom=-np.inf for infinite depth.
...
DEPTH FIX: BEM problems now include sea_bottom to account for finite water depth.
           With spar draft ~1.4 m and tank depth 2.4 m, the ~1 m clearance to the
           bottom affects radiation coefficients for heave. Infinite depth assumption
           underestimates added mass and overestimates radiation damping of the spar.
"""
...
def analyze_two_body_hydrodynamics(mesh1_path, mesh2_path, frequency_range, 
    ...
    sea_bottom : float
        Sea bottom depth [m], negative downward (e.g. -2.4 for 2.4 m tank).
        Use -np.inf for infinite depth (original behavior).
        Default: -2.4 m (wave tank depth).
    ...
    dict : Analysis results containing:
        ...
        - sea_bottom: Water depth used in analysis
    """
    
    # Disable Capytaine progress display to avoid LiveError in loops
    import logging
    logging.getLogger('capytaine').setLevel(logging.CRITICAL)
    logging.getLogger('capytaine').propagate = False

    # cpt.set_logging() activates a rich Live display that crashes in batch loops.
    # Only enable it for interactive/debug use — never in CRITICAL or ERROR mode.
    if logging_level.upper() not in ("CRITICAL", "ERROR"):
        try:
            cpt.set_logging(level=logging_level)
        except Exception:
            pass  # Silently ignore if Live display is already active

    # Report water depth condition
    if np.isinf(sea_bottom):
        print(f"🌊 Water depth: INFINITE (deep water assumption)")
    else:
        water_depth = abs(sea_bottom)
        print(f"🌊 Water depth: {water_depth:.2f} m  (sea_bottom = {sea_bottom:.2f} m)")
        print(f"   ⚠️  Finite depth BEM — panel method includes seabed boundary condition")
    ...
    # Validate clearance between spar bottom and sea_bottom
    if not np.isinf(sea_bottom):
        min_z_body1 = np.min(mesh1.vertices[:, 2])
        min_z_body2 = np.min(mesh2.vertices[:, 2])
        min_z = min(min_z_body1, min_z_body2)
        clearance = min_z - sea_bottom
        print(f"\n📏 Geometry / depth check:")
        print(f"   Lowest mesh point: z = {min_z:.3f} m")
        print(f"   Sea bottom:        z = {sea_bottom:.3f} m")
        print(f"   Clearance:         {clearance:.3f} m")
        if clearance < 0.05:
            print(f"   ⚠️  WARNING: Very small clearance ({clearance:.3f} m). "
                  f"BEM accuracy may be reduced — consider refining mesh near keel.")
        elif clearance < 0.2:
            print(f"   ⚠️  NOTE: Clearance is small ({clearance:.3f} m). "
                  f"Finite depth effect on radiation coefficients will be significant.")
        else:
            print(f"   ✅ Clearance is adequate for BEM analysis.")
    ...
    for omega in w:
        # Radiation problems for each DOF — with sea_bottom
        ...
        # Diffraction problem — with sea_bottom
    ...
    hydro_coeffs = {
        ...
        'w': w,                          # Frequencies
        'sea_bottom': sea_bottom,        # Water depth used
        'metadata': {
            ...
            'sea_bottom': float(sea_bottom) if not np.isinf(sea_bottom) else 'infinite',
            'water_depth_m': float(abs(sea_bottom)) if not np.isinf(sea_bottom) else 'infinite',
    ...
    try:
        ...
        print(f"✅ HydCoeff.npz saved successfully")
    ...
    try:
        ...
        print(f"✅ HydCoeff.pkl saved successfully")
    except Exception as e:
    ...
    try:
        ...
        print(f"✅ HydCoeff.mat saved successfully")
    except Exception as e:
    ...
    if save_plots or show_plots:
        ...
        depth_label = f"h = {abs(sea_bottom):.1f} m" if not np.isinf(sea_bottom) else "h = ∞"
        plt.title(f"Radiation Damping Coefficients (diagonal terms) [{depth_label}]")
        ...
        plt.title(f"Added Mass Coefficients (kg) [{depth_label}]")
        plt.xlabel("Wave frequency [rad/s]")
        ...
        # Draw sea bottom line if finite depth
        if not np.isinf(sea_bottom):
            ax.axhline(y=sea_bottom, color='saddlebrown', linewidth=2.5,
                       linestyle='--', label=f'Sea bottom (z = {sea_bottom:.2f} m)')
        ...
        ax.set_xlabel("X [m]")
        ax.set_ylabel("Z [m]")
        ax.set_title(f"Lateral view (XZ plane): {body_names[0]} and {body_names[1]} [{depth_label}]")
        ax.set_aspect('equal')
        ax.grid(True)
        plt.tight_layout()
    ...
    print(f"   - Water depth (sea_bottom): {sea_bottom} m")
...
if __name__ == "__main__":
    ...
    results = analyze_two_body_hydrodynamics(
        ...
        output_directory="hydroData",
        nc_filename="rm3.nc",
        sea_bottom=-2.4          # Wave tank depth [m]
    )
    ...
    print(f"   - Total panels: {Npan1 + Npan2}")
    print(f"   - Water depth: {abs(results['sea_bottom']):.2f} m")
